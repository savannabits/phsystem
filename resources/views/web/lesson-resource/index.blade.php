...
@section('title', trans('admin.lesson-resource.actions.index'))

@section('body')

    <lesson-resource-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('lesson-resources') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.lesson-resource.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('lesson-resources/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson-resource.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block table-responsive">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.lesson-resource.columns.id') }}</th>
                                        <th is='sortable' :column="'title'">{{ trans('admin.lesson-resource.columns.title') }}</th>
                                        <th is='sortable' :column="'description'">{{ trans('admin.lesson-resource.columns.description') }}</th>
                                        <th is='sortable' class="text-center" :column="'published_at'">{{ trans('admin.lesson-resource.columns.published_at') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.lesson-resource.columns.enabled') }}</th>
                                        <th is='sortable' :column="'uploaded_by'">{{ trans('admin.lesson-resource.columns.uploaded_by') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('savannabits/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/lesson-resources')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('savannabits/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('https://csm.strathmore.edu/ph/lesson-resources/bulk-destroy')">{{ trans('savannabits/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <td>@{{ item.id }}</td>
                                        ...
                                        <td>@{{ item.description }}</td>
                                            ...
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{ trans('admin.lesson-resource.actions.will_be_published') }}</small><br />
                                                @{{ item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('lesson-resources/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson-resource.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
