...
@section('title', trans('admin.child.actions.index'))
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.child.columns.id') }}</th>
                                        <th is='sortable' :column="'first_name'">{{ trans('admin.child.columns.first_name') }}</th>
                                        <th is='sortable' :column="'middle_names'">{{ trans('admin.child.columns.middle_names') }}</th>
                                        <th is='sortable' :column="'last_name'">{{ trans('admin.child.columns.last_name') }}</th>
                                        <th is='sortable' :column="'gender'">{{ trans('admin.child.columns.gender') }}</th>
                                        <th is='sortable' :column="'dob'">{{ trans('admin.child.columns.dob') }}</th>
                                        <th is='sortable' :column="'school'">{{ trans('admin.child.columns.school') }}</th>
                                        <th is='sortable' :column="'active'">{{ trans('admin.child.columns.active') }}</th>
                                        <th is='sortable' :column="'enrollment_date'">{{ trans('admin.child.columns.enrollment_date') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="11">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('savannabits/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/children')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('savannabits/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('https://csm.strathmore.edu/ph/children/bulk-destroy')">{{ trans('savannabits/admin-ui::admin.btn.delete') }}</button>
                                            </span>
