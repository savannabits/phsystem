            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('lessons/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block table-responsive">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'ph_class_id'">{{ trans('admin.lesson.columns.ph_class_id') }}</th>
                                        <th is='sortable' :column="'facilitator_id'">{{ trans('admin.lesson.columns.facilitator_id') }}</th>
                                        <th is='sortable' :column="'lesson_date'">{{ trans('admin.lesson.columns.lesson_date') }}</th>
                                        <th is='sortable' :column="'start_time'">{{ trans('admin.lesson.columns.start_time') }}</th>
                                        <th is='sortable' :column="'end_time'">{{ trans('admin.lesson.columns.end_time') }}</th>
                                        <th is='sortable' :column="'objective'">{{ trans('admin.lesson.columns.objective') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.lesson.columns.enabled') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="10">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('savannabits/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/lessons')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('savannabits/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('https://csm.strathmore.edu/ph/lessons/bulk-destroy')">{{ trans('savannabits/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('lessons/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
