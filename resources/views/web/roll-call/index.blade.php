...
@section('title', trans('admin.roll-call.actions.index'))
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.roll-call.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('roll-calls/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.roll-call.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block table-responsive">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.roll-call.columns.id') }}</th>
                                        <th is='sortable' :column="'date'">{{ trans('admin.roll-call.columns.date') }}</th>
                                        <th is='sortable' :column="'ph_class_id'">{{ trans('admin.roll-call.columns.ph_class_id') }}</th>
                                        <th is='sortable' :column="'created_by'">{{ trans('admin.roll-call.columns.created_by') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('savannabits/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/roll-calls')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('savannabits/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('https://csm.strathmore.edu/ph/roll-calls/bulk-destroy')">{{ trans('savannabits/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('roll-calls/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.roll-call.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
